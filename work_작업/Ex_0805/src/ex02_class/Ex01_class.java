...
//프로그램을 실행했을 때, 코드를 컴파일한 결과물은 코드 파일을
//각각 작성한것과 동일하게 각 class별로 도출되어 2개가 된다.
//파일 분리 여부와 상관없이 결과물이 같기 때문에, 분리 여부는 개발자가
//원흐는데로 작성해도 무방하다.
//그러나, 추후 유지보수의 편리성과 클래스 재사용을 고려해
//하나의 파일에 한개의 클래스를 작성하는 것을 추천한다.
...
public class Ex01_class {
	public static void main(String[] args) {
		...
		//메서드를 선언한 클래스 안에서 메서드를 사용할 때는
		//단순히 메서드명만 호출하면 되지만
		//다른 클래스에서 메서드를 사용하려면
		//객체를 생성한 후 참조 변수를 이용해
		//그 객체의 메서드를 사용해야한다.
		...
		//값을 돌려주는 것을 '반환한다'라고 표현하고,
		//반환하는 결과 값을 '반환값'이라고 한다.
		//반환값이 있는 경우 반환하는 데이터의 타입을 반환형에 명시해야 한다.
		//int, boolean등 기본자료형과 String같은 참조 자료형도 가능하지만
