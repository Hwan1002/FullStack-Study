...
public class Ex_Variable01 {
	...
	public static void main(String[] args) {
		...
		//변수명 명명 규칙
		//사용 용도츨 최대한 명확하게 보여줄 수 있는 이름이라면
		//개발자가 마음대로 지정할 수 있지만 다음과 같은 규칙을 지켜야한다.
		//1. 숫자가 먼저 들어가면 안된다.
		//2. 영어 대소문자를 구별한다.(Name과 name은 다른 변수라고 인식)
		//3. 일반적으로 영어 소문자로 시작한다.
		//4. 언더바 _, 달러 $ 이외의 특수문자는 사용할 수 없다.
		//5. 예약어 금지(if, for, while, switch new 등등)
		//6. 한글은 쓰지 말것 (하지만 가능은함)
		...
		//표기법
		//카멜표기법(camel case)
		//두번째 단어부터 첫글자를 대문자로 표기하는 방식
		//example) userName, phoneNumber
		
		//스네이크 표기법 (snake case)
		//단어가 소문자로 시작하고, 단어와 단어 사이는 _로 연결하는 방식
		//example) user_name, phone_number
		
		//변수에 데이터 입력하기
		//개발자가 데이터 값이 필요할 때 데이터의 값을 직접 사용하는 대신,
		...
		//변수를 선언하고 초기화 하는 작업을 한줄로 쓸 수 있다.
		String myCity = "SEOUL";
