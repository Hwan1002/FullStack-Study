...
class AddThread implements Runnable{
	...
	@Override
	public void run() {
		synchronized (b) {//n번이 반복문을 다돌때까지 다음n번이 나오지않는다
			try {
				for(int i=0; i<10; i++) {
...
public class BankThreadExample {
	public static void main(String[] args) {
		...
		//그 이유는 두 개의 스레드가 서로의 작업에 간섭하여 정상적으로 입급되지 않았기 때문이다.
		//이러한 오류를 방지하기 위해 여러 스레드가 하나의 공유 데이터에
		//동시에 접근하지 못하도록 스레드의 실행을 제어하는 '스레드 동기화'가 필요하다.
	}
}
