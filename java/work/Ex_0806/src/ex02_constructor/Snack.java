...
public class Snack {
	...
	//클래스를 생성하면서 개발자가 직접 생성자를 선언하지않았지만,
	//자바 컴파일러가 기본생성자를 자동으로 제공해주고있다.
	//다만,컴파일러의 눈에만 보일뿐 우리가 보는 코드에는 생략되어있다.
	int price;
	
	//기본 생성자
	//매개변수가 별도로 없으며, 중괄호 블록 안에 코드가 없는 비어있는 생성자를 말한다.
	//기본 생성자는 개발자가 직접 선언하지 않았을 때만 컴파일러가 자동으로 추가한다.
...
//	}
	...
	//객체를 사용하기위해 메모리에 올린다는 의미이다.
	//생성자를 통해 객체를 메모리에 올림과 동시에, 더 나아가 객체 멤버에 접근이 가능하므로
	//일반적인 메서드 처럼 객체 멤버의 데이터를 초기화 할 수 있다.
